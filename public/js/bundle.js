...
var displayMap = exports.displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiYW1pdHNoaXZheXBhdGVsIiwiYSI6ImNtZW81Zmg0dDB5cGgya3M4cmswODB1bDUifQ.QfBsPbr0aOIc5HaC07pPPg';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/amitshivaypatel/cmeo5hg6i00th01sdcxglgkyl',
    scrollZoom: false
...
document.addEventListener('DOMContentLoaded', function () {
  var starfield = document.getElementById('starfield-background');
  if (starfield) {
    // Create stars
    var starCount = 200;
    for (var i = 0; i < starCount; i++) {
      ...
      var twinkleDuration = 2 + Math.random() * 8;
      var opacity = 0.2 + Math.random() * 0.8;

      // Apply properties
      star.style.width = "".concat(size, "px");
      star.style.height = "".concat(size, "px");
      star.style.left = "".concat(posX, "%");
      star.style.top = "".concat(posY, "%");
      star.style.setProperty('--twinkle-duration', "".concat(twinkleDuration, "s"));
      star.style.opacity = opacity;

      // Add variation to twinkling
      if (twinkleDuration > 6) star.classList.add('slow');else if (twinkleDuration < 4) star.classList.add('fast');
    ...
    document.addEventListener('mousemove', function (e) {
      ...
      stars.forEach(function (star) {
        var speed = parseFloat(star.style.width) * 0.5;
        var xOffset = mouseX * speed * 20;
        var yOffset = mouseY * speed * 20;
        var currentLeft = parseFloat(star.style.left);
        var currentTop = parseFloat(star.style.top);
        var newLeft = currentLeft + xOffset * 0.05;
        var newTop = currentTop + yOffset * 0.05;

        // Keep stars within bounds
        star.style.left = "".concat(Math.max(0, Math.min(100, newLeft)), "%");
        star.style.top = "".concat(Math.max(0, Math.min(100, newTop)), "%");
      });
    });
  }
});
